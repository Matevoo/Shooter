...
static bool entityCollision(Entity *target) {
    for (Entity *fighter = core.fighterHead.next; fighter != NULL; fighter = fighter->next)
        if (fighter->side != target->side && didCollide(
            ...
            addExplosions(fighter->pos.x, fighter->pos.y, 32);
            addDebris(fighter);
            return true;
        }
    return false;
...
void moveFighters() {
    ...
    for (fighter = core.fighterHead.next; fighter != NULL; fighter = fighter->next) {
        fighter->pos.x += fighter->pos.dx, fighter->pos.y += fighter->pos.dy;
...
void moveBullets() {
    ...
    for (bullet = core.bulletHead.next; bullet != NULL; bullet = bullet->next) {
        bullet->pos.x += bullet->pos.dx, bullet->pos.y += bullet->pos.dy;
        if (entityCollision(bullet) || bullet->pos.x <= -bullet->w || bullet->pos.y <= -bullet->h ||
...
void moveExplosions() {
    ...
    for (explosion = core.explosionHead.next; explosion != NULL; explosion = explosion->next) {
        explosion->pos.x += explosion->pos.dx, explosion->pos.y += explosion->pos.dy;
        if (--explosion->a <= 0) {
...
void moveDebris() {
    ...
    for (debris = core.debrisHead.next; debris != NULL; debris = debris->next) {
        debris->pos.x += debris->pos.dx, debris->pos.y += debris->pos.dy, debris->pos.dy += 0.4;
        if (--debris->life <= 0) {
